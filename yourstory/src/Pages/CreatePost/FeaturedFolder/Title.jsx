import React,{useState} from "react"
import Grid from "@material-ui/core/Grid"
import TextareaAutosize from '@material-ui/core/TextareaAutosize';
import Button from '@material-ui/core/Button';
import Tooltip from '@material-ui/core/Tooltip';
import styles from "./Title.module.css"
const Title = () => {
    const [title, setTitle] = useState("")

    const handleChnage = (e) => {
        if(title.split("").length <= 1000){
            setTitle(e.target.value)
        }
    }

    const handleKeyUp = (e) => {
        if(title.split("").length > 1000 && e.keyCode === 8){
            setTitle(e.target.value)
        }
    }

    return (
        <Grid container  >
            {/* <Grid item xs = {12} sm = {6} md = {5} className = { styles.margin50Bottom0} > */}
            <Grid item style= {{width:"50%"}} className = { styles.margin50Bottom0} >
                Title* <Tooltip title="This is the title of the story. This is a mandatory field" arrow>
                        <Button>
                        <img className = {styles.titleSvg} src="data:image/png;base64," alt="titlesvg"/>
                        </Button>
                        </Tooltip>
                <textarea value = {title} className = {styles.titleTextArea} rows={6} onKeyUp = {handleKeyUp} onChange = {handleChnage} placeholder = "Title of the story" />
                <div className = {styles.displayFlexSpaceBitween}>
                    <div className = {styles.colorRed}>* Title of the story must be provided</div>
                    <div className = {styles.colorCyan}>{title.split("").length}/1000</div>
                </div>
            </Grid>
        </Grid>
    )
}



export default Title